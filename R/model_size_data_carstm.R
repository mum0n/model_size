...
model_size_data_carstm = function(p, sexid=NULL,  redo=c(""), tokeep="necessary") { 
  ...
  if (!is.null(sexid)) {
    ...
    fn = file.path( 
      p$project.outputdir, 
      paste( "size_distributions_tabulated_data_zeros_", sexid, "_", paste0( span, collapse="_"), ".rdz", sep="" )  
    )
  ...
  for ( sexid in c("female", "male") ) {
    ...
    detcm = detcm[ sex %in% sexcd[[sexid]] , ]  
    ...
    o = lm( log(mass) ~ logcw + factor(mat), detcm)
